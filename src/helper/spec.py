...
def CalcSpec(mol,func):
    ...
    for i in Files:
        ...
        if ("a" in [x['spin'] for x in orbitals]):
            psi4.core.print_out("\nALPHA orbitals")
            orbI = ([c for c,x in enumerate(occa) if x != 1.0][0])
            psi4.core.print_out("\nInitial Orbital: {:d}".format(orbI))
            Ci = Ca[orbI]
            orbF = ([c for c,x in enumerate(occa) if (x != 1.0) and (c!=orbI)])
            psi4.core.print_out("\nFinal Orbitals: {} \n\n".format(str(orbF)))
            Cf = Ca[orbF]
            ...
            if "Aug" in i:
                with open(prefix+'_aAug.spectrum', 'wb') as handle:
                    pickle.dump(spec, handle, protocol=pickle.HIGHEST_PROTOCOL)
                psi4.core.print_out(("\n{}"+"_aAug.spectrum written.. \n\n").format(prefix))
            else:
                with open(prefix+'_a.spectrum', 'wb') as handle:
                    pickle.dump(spec, handle, protocol=pickle.HIGHEST_PROTOCOL)
                psi4.core.print_out(("\n{}"+"_a.spectrum written.. \n\n").format(prefix))
